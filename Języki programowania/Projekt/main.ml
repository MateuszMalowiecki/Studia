...
let checkbinding fi ctx exc_ctx b = match b with
    NameBind -> NameBind
  | VarBind(tyT) -> VarBind(tyT)
  | TmAbbBind(t,None) -> TmAbbBind(t, Some(typeof ctx exc_ctx t))
...
let prbindingty ctx exc_ctx b = match b with
  ...
  | TmAbbBind(t, tyT_opt) -> pr ": ";
     (match tyT_opt with
         None -> printty ctx (typeof ctx exc_ctx t)
       | Some(tyT) -> printty ctx tyT)

let rec process_file f ctx exc_ctx =
  if List.mem f (!alreadyImported) then
    ...
    let g ctx c =  
      open_hvbox 0;
      let results = process_command ctx exc_ctx c in
      print_flush();
      results
    in
...
and process_command ctx exc_ctx cmd = match cmd with
  ...
  | Bind(fi,x,bind) -> 
      ...
      pr x; pr " "; prbindingty ctx exc_ctx bind'; force_newline();
