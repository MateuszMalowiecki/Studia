...
case class BlackJack(deck: Deck, aceValue: Int = 11) {
  ...
  val magicNumber: Int = 21

  def pointsFromCard(card: Card): Int = {
    card.value match {
      case Ace => aceValue
      case _: Face => 10
      case Numerical(i) => i
  ...
  def play(n: Int): Unit = {
    ...
    val nCards = deck.cards.take(n)
    val sum = nCards.foldLeft(0)((part_sum, card) => printAndAddToSum(part_sum, card))
    println(sum)
  }
    ...
    def subsequences(list: List[Card]): List[List[Card]] = {
      list match {
        ...
        case head :: tail => {
          ...
          allsubSeq.filter(xs => pointsFromList(xs) <= magicNumber)
        }
      }
    }

    subsequences(deck.cards).filter(p => pointsFromList(p) == magicNumber)
  }
...
object BlackJack {
  ...
  def apply(numOfDecks: Int): BlackJack = {
    ...
    val decks = for (_ <- 1 to numOfDecks) yield shuffle(standardDeck)
